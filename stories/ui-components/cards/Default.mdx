...
- The grid container has an extra `kb-card-grid` class to override the default CSS Grid behaviour of `kb-grid`. See [Grid layout story](/story/ui-components-layouts-grid--grid).
- For example, the `kb-card-grid` column min-widths are overridden by:
```
.kb-card-grid.kb-grid { @include responsive-grid-columns($min-width: 17.5rem) }
```
