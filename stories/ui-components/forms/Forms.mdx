...
- Additionally, on server postback, form errors are identified by an extra `kb-form__field--has-error` class.
...
## Disabled fields
- In addition to the form control having a `disabled` attribute, the parent `kb-form__field` or `kb-form__fieldset` has an `is-disabled` class.
