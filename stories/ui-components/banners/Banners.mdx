...
- Hero banners can also be referred to as **billboards**. They tend to be on homepages, but can be used anywhere.
...
- The slider also has a CSS `kb-slider--banner` class to identify it specifically as a banner slider.
...
## Page title banner
- Page title banners are normally at the top of the page, and should therefore contain the `<h1>` page title.
- The title is not rendered in an overlay so the image is not obscured.

<Preview>
    <Story id="ui-components-banners--banner-page-title" />
</Preview>
